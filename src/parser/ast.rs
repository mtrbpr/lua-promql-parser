...
impl IntoLua for Expr {
    fn into_lua(self, lua: &mlua::Lua) -> mlua::Result<mlua::Value> {
        let obj: Option<serde_json::Map<String, Value>> = json!(self).as_object().cloned();
        Self::obj_to_lua(obj, lua)
    }
}
...
impl Expr {
    fn obj_to_lua(
        ...
        for (key, val) in obj.unwrap_or_default().into_iter() {
            result.insert(key, Expr::to_lua(val, lua));
        }
        lua.create_table_from(result.into_iter())
            .map(|x| mlua::Value::Table(x))
    }
    fn to_lua(exp: Value, lua: &mlua::Lua) -> mlua::Value {
        match exp {
            Value::Null => LuaValue::Nil,
            Value::Array(a) => LuaValue::Table(
                lua.create_table_from(a.iter().map(|x| Self::to_lua(x.clone(), lua)).enumerate().map(|(k,v)| (k+1, v)))
                    .unwrap_or(lua.create_table().unwrap()),
            ),
            Value::Bool(b) => LuaValue::Boolean(b),
            Value::Number(n) => LuaValue::Number(n.as_f64().unwrap_or_default()),
            Value::Object(o) => Self::obj_to_lua(Some(o), lua).unwrap_or(LuaValue::Nil),
            Value::String(s) => LuaValue::String(Lua::create_string(lua, s).unwrap()),
        }
    }
